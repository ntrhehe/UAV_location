...
- Labelme JSON 标注 → YOLO 格式转换

## 总体思路

`
JPG (含XMP位姿) ──→ 相机光线方向向量 ──→ ENU 坐标系
                          │
                          ▼
                   ECEF 空间射线步进
                          │
...
**用法：**
`ash
py src/pixel_to_geo_dem.py <照片.JPG> <DSM.tif> <像素u> <像素v>
`
...
**用法：**
`ash
py src/geo_reference.py --img <照片目录> --dsm <DSM.tif> --out <输出目录> [--cam cam.txt] [--csv pos.csv]
`
...
**用法：**
`ash
py src/image_footprint.py <照片.JPG> <DSM.tif> [--cam cam.txt] [--csv pos.csv]
`
...
**用法：**
`ash
py src/full_coverage_selector.py --img <照片目录> --dsm <DSM.tif> --shp <ROI.shp> --out <输出目录>
`
...
将 Labelme JSON 标注文件转换为 YOLO 格式的 txt 文件。修改脚本中的 IMG_DIR 和 CLASS_MAP 后直接运行。
...
### src/_write_gr.py — 代码生成辅助

内部辅助脚本，用于自动生成 geo_reference.py 的模板代码。
